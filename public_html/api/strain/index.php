...
try {
	...
	if($method === "GET") {
		...
		} elseif(empty($strainName) === false) {
			$strain = Strain::getStrainByStrainName($pdo, $strainName);
			if($strainName !== null) {
				$reply->data = $strain->toArray();
			}
		} else {
			$strains = Strain::getAllStrains($pdo);
			if($strains !== null) {
				$reply->data = $strains->toArray();
			}
		}
	} else{
