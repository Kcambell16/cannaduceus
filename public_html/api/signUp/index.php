...
try {
	...
	if($method === "POST") {
		...
		if(empty($requestObject->profileFirstName) === true) {
			...
			$profileFirstName = filter_var($requestObject->profileFirstName, FILTER_SANITIZE_STRING, FILTER_FLAG_NO_ENCODE_QUOTES);
		}
		if(empty($requestObject->profileLastName) === true) {
			throw(new \InvalidArgumentException ("Must fill in valid last name", 405));
		} else {
			$profileLastName = filter_var($requestObject->profileLastName, FILTER_SANITIZE_STRING, FILTER_FLAG_NO_ENCODE_QUOTES);
		}
		if(empty($requestObject->profileProfileName) === true) {
			throw(new \InvalidArgumentException ("Must input valid profile name", 405));
		} else {
			$profileProfileName = filter_var($requestObject->profileProfileName, FILTER_SANITIZE_STRING, FILTER_FLAG_NO_ENCODE_QUOTES);
		}
		...
		$profile = new Cannaduceus\Profile(null, null, $profileAccessLevel, $profileActivationToken, $profileEmail,
			$profileFirstName, $hash, $profileLastName, $salt, $profileProfileName);
		$profile->insert($pdo);
...
EOF;
		$response = sendEmail($profileEmail, $profileFirstName, $profileLastName, $messageSubject, $message);
